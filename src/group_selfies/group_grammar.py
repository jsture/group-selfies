...
from group_selfies.group_mol_graph import Group, GroupWrapper
from group_selfies.constants import POP_SYMBOL, INDEX_CODE
import numpy as np
...
class GroupGrammar:
    ...
    def from_list(cls, group_list):
        vocab_dict = dict(
            [
                ...
                for n, s, all_at, *priority in group_list
            ]
        )
    ...
    def from_file(cls, filename):
        test_gs = [line.strip().split(" ") for line in open(filename)]
        vocab_dict = dict(
            [
    ...
    def essential_set(cls):
        ...
        test_gs = [ess.strip().split(" ") for ess in essential]
        vocab_dict = dict(
            [
                (n, Group(n, s, priority=int(priority[0]) if len(priority) else 0))
                for n, s, *priority in test_gs
    ...
    def extract_groups(self, mol):
        ...
        for name in self.names_sorted_by_size():
            ...
            for matched_atoms, matched_bonds in extract_group_bonds(
                ...
                if _compatible(matched_atoms, mapped_atoms, group.mol, mol):  # type: ignore TODO: fix whether group.mol is indeed defined or important
